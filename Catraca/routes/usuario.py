...
def usuario_recarregar_cartao():
    cartao, erro = cartao_usuario_atual_ou_erro(g.acesso.get("usuario_id"))
    sucesso = None
    valor_escolhido = ""
        ...
        if valor is None or valor <= 0:
            ...
            try:
                ...
                if not cartao_atual:
                    ...
                    saldo_anterior = Decimal(str(cartao_atual["saldo"])).quantize(
                        Decimal("0.01")
                    )
...
def usuario_planejar_viagem():
    usuario_id = g.acesso.get("usuario_id")
    cartao = cartao_do_usuario(usuario_id) if usuario_id else None
    if not cartao:
        return render_template(
        ...
        if acao == "adicionar":
            ...
            if not trechos:
                ...
                try:
                    ...
                    return redirect(url_for("usuario_minha_viagem"))
                except Exception:
                    db.execute("ROLLBACK")
